...
%%
x_original = 1:numel(height_real_raw);
x_interp = linspace(1, numel(height_real_raw), 881);
height_real_intp = interp1(x_original, height_real_raw, x_interp, 'linear');
height_real=smoothdata(height_real_intp,"gaussian",50);
height_sim_smooth = smoothdata(modiBl_height, "gaussian", 60);
height_sim = height_sim_smooth+600;
plot(ctime,height_sim,'LineWidth',2.5)
hold on
plot(ctime,height_real ,'LineWidth',2.5)
xlabel("Time")
ylabel("Height(m)")

save ./Mat_Files/1_ctime.mat ctime
save ./Mat_Files/1_height_sim.mat height_sim
save ./Mat_Files/1_height_real.mat height_real
