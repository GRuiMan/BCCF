...
load ./Mat_Files/Boundary_layer_height_cal.mat
load ./Mat_Files/BC_vary.mat
...
plot(ctime1,BC2,'LineStyle','-','LineWidth',1.5)
hold on

% ct1=smoothdata(ct1, "gaussian", 50);
hold on
plot(ctime1,ct1,'LineStyle','-','LineWidth',1.5)

ylabel("Concentration/(ng/m^{-3})")
legend("Real","Simulated")
box off
set(gca, 'FontName', 'Times New Roman')
saveas(gcf,"./Figures/真实黑炭数据与模拟黑炭数据.svg")
...
hold on
plot(ctime1(1:choose),smoothdata(ratio2,'gaussian',200),'LineWidth',1.5,'Color',[0.4660 0.6740 0.1880])
hold on
plot(ctime1(1:choose),ratio3,'LineWidth',1.5)
hold on
% plot(ctime1,ones(1,881),'Color','k','LineWidth',1.)
ylabel("$Ratio:\frac{simulated}{real data}$","Interpreter","latex")
...
legend("considering all factors", ...
    ...
    'Location', 'northwest')
set(gca, 'FontName', 'Times New Roman')
saveas(gcf,"./Figures/Figure5_对不同因素考虑与排除.svg")
